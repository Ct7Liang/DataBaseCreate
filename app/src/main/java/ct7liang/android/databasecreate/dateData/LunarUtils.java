...
public class LunarUtils {
    ...
    public LunarUtils(Calendar cal) {
        ...
        int yearCyl, monCyl, dayCyl, leapMonth;
        Date baseDate = null;
        try {
     ...
     */
     private static int yearDays(int y) {
        int i, sum = 348;   //348为 12个阴历月,每个月的基本天数为29天
    ...
    /**
     * 地球绕太阳运行周期为365天5小时48分46秒 即一回归年: 365d 5h 48m 46s
     * 因此每年少过 5h 48m 46s
     * 累计四年 即 5h 48m 46s * 4
     * 四年少过 23h 15m 4s
     * 然后采用闰年的方式在第四年补一天, 即为二月二十九日
     * 所以是每四年一个闰年
     * 但是这样一来实际上每四年又多过了 44m 56s
     * 然后累计400年出来, 也就是累计100个闰年后
     * 又多过了 44m 56s * 100 = 74h 53m 20s
     * 相当于三天的时间 72h
     * 所以又从100个闰年里面抠出来3天
     * 最后规定: 闰年为每四年一年 (年份数可以被4出尽即为闰年) 但是逢整百的年份需要被400除尽 才是闰年
     *
     * 判断是否是闰年:
     *  if(年份数是否能被100整除){
     *      if(能被400整除){
